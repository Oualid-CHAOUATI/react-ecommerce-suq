...
type TScheme = "dark" | "light";

function useScheme() {
  const localStorageKey = "suuq-scheme";

  const [scheme, setScheme] = useState<TScheme | null>(null);

  if (scheme == null) {
    let currentScheme: TScheme | null;
    ...
    currentScheme = localStorage.getItem(localStorageKey) as TScheme | null;
    currentScheme = currentScheme || (prefersDarkScheme ? "dark" : "light");
    setScheme(() => {
      setUiScheme(currentScheme!);
      return currentScheme;
    });
  }

  // const [scheme, setScheme] = useState<TScheme>(currentScheme);

  function toggleScheme() {
    const newScheme: TScheme = scheme == "dark" ? "light" : "dark";
    setUiScheme(newScheme);
    setScheme(newScheme);
  }

  function setUiScheme(scheme: TScheme) {
    document.documentElement.setAttribute("color-scheme", scheme!);
    localStorage.setItem(localStorageKey, scheme!);
  }

  return [scheme, toggleScheme] as [TScheme, () => void];
}
